var SimpleToken1 = artifacts.require("../contracts/SimpleToken1");
var send = require('./TimeTravel.js');

contract('SimpleToken1', function(accounts) {
    var stInstance;

    SimpleToken1.deployed().then(token => {
    ...
    it("Passes testcase 2 ", async function() {
        let token = await stInstance;
        await token.addAgent(accounts[1], 10 ** 18 * 900000000);
        await send('evm_mine');
    ...
    it("Passes testcase 3 ", async function() {
        ...
        let token = await stInstance;
        await token.addAgent(accounts[0], 10 ** 18 * 3000000000);
        await send('evm_mine');
